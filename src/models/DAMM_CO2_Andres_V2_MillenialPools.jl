# V2 DAMM - Millenial Pools version of CO2 model (tailored for peatlands (without MAOM and Aggregate pools)))
...
C_total_initial = 500       # g C m^-2 (initial total SOC stock)
...
LMWC0     = 50.0    # g C m^-2, LMWC (DOC pool)
...
function soc_oxic_damm!(du, u, p_, t)
    ...
    Sx = LMWC * Dliq_t(t) / depth          # [Sx] g C m-3 (soluble fraction of SOC) substrate concentration over time
    O2 = Dgas_t(t) * O2airfraction      # How much O2 diffuses and 4/3 is how much is at microbial sites

    # Temperature effects
    TK = T_C(t) + 273.15                  # Kelvin conversion
    Vmax = αS_ref * exp(-Ea/Rgas * (1/TK - 1/Tref)) # Arrhenius function for temperature dependence of reaction rates
    kM_Sx = ckM + mkM * T_C(t)            # g C m^-3. Michaelis constant for substrate with linear T dependence.

    # DAMM rate on both substrates
    Max_M_Assimilation = Vmax * (Sx / (kM_Sx + Sx)) * (O2 / (kM_O2 + O2))  # Potential Flux from LMWC to Microbial Biomass (controlled by kinetics)
    F_lm = Max_M_Assimilation * MB       # Actual Flux from LMWC to Microbial Biomass (scaled by microbial biomass)

    # Fluxes between pools
    F_POM_LMWC = k_PL * POM                # Flux of POM into LMWC (first-order kinetics)
    F_MB_LMWC = k_MB * MB                  # C Flux of Microbial biomass back into the LMWC pool

    # Respiration (CO2 flux)
    Rco2_total = (1 - CUE) * F_lm

    # Differential equations for each pool
    du[1] = NPP_t(t) - F_POM_LMWC             # Change in POM pool: NPP input minus decomposition to LMWC
    du[2] = F_POM_LMWC + F_MB_LMWC - F_lm        # Change in LMWC pool: Input from POM and MB turnover minus uptake by MB
    du[3] = CUE * F_lm - F_MB_LMWC           # Change in MB pool: Growth minus turnover
    du[4] = 0.0                         # MAOM pool remains at zero
    du[5] = Rco2_total                  # Cumulative CO2 flux output
end
...
# Calculate total C stock for plotting
...
# Panel 1: SOC stock
p1 = plot(t, SOC, lw=2, xlabel="Day", ylabel="Total SOC (g C m⁻²)", label="Total SOC", title="Total SOC stock over time")
